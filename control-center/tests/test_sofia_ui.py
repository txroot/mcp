...
const TOKEN='test';
function notify(){}
function refresh(){const d={items:[]};document.getElementById('grid').innerHTML=d.items.map(card).join('');}
...
def test_upgrade_adds_reconciled_domains_health_and_fail_closed_lifecycle():
    ...
    assert "function domainComponent" in html
    assert "function domainCard" in html
    assert "function renderGrid" in html
    assert "payload.runtime_inventory" in html
    assert "Canonical Sofia OS domain" in html
    assert "lifecycle disabled during reconciliation" in html
    assert "document.getElementById('grid').innerHTML=renderGrid(d);" in html
